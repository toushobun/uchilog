...
begin
    ...
    v_balance_delta := case when p_type = 'expense' then -p_amount else p_amount end;
    ...
    insert into public.transaction_item (
        ...
        p_ledger_id, v_transaction_record_id, p_account_id, p_category_id, p_type,
        p_amount, 0, v_balance_delta, null, 0,
        v_user_id, v_user_id
    );
...
grant execute on function public.create_transaction(
    uuid, text, timestamptz, numeric, uuid, uuid, uuid, text
) to authenticated;
