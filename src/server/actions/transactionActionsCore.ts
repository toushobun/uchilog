...
function parseRawTransactionType(formData: FormData) {
  const rawType = String(formData.get("type") ?? "").trim();

  return transactionRecordTypes.find((type) => type === rawType);
}

function newTransactionValidationErrorHref(error: string, formData: FormData) {
  return newTransactionErrorHref(error, parseRawTransactionType(formData));
}
...
export async function createTransaction(formData: FormData) {
  ...
  if (!result.ok) {
    redirect(newTransactionErrorHref(result.error, values.type));
  }
